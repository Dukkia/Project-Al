...
const cache = require('../default.gg/cache'); // cache.js 모듈 불러오기
...
const createServer = (port, targetLang) => {
    ...
    app.get('/', async (req, res) => {
        try {
            ...
            const response = await axios.get('https://api.performfeeds.com/soccerdata/match/1wajy57wfq6wo1qnta55rgx3an?_rt=b&_fmt=json&tmcl=8dz245skul7xcviumzumhlkwk&lineups=yes&live=yes&mt.mDt=[2024-04-01T00:00:00Z%20TO%202024-05-31T23:59:59Z]');
            const data = response.data;

            // data.match가 존재하는지 확인
            if (!data || !data.match) {
            ...
            const formattedData = await Promise.all(matches.map(async (match) => {
                ...
                const translatedLineUp = await Promise.all((match.liveData.lineUp || []).map(async (lineUpItem) => {
                    const translatedPlayers = await Promise.all((lineUpItem.player || []).map(async (player) => {
                        ...
                        return {
                            ...player,
                            matchName: translatedMatchName || player.matchName // 번역 실패 시 원본 이름 사용
                        };
                    }));
