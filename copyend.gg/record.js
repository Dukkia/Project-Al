...
const createServer = (port, targetLang) => {
    ...
    app.get('/', async (req, res) => {
        try {
            ...
            // record.json 파일에서 데이터를 읽어옴
            const data = JSON.parse(fs.readFileSync('../data.gg/record.json', 'utf8'));

            let translatedData = data;
            if (targetLang !== 'en') {
                translatedData = await translateContestantNames(data, targetLang);
            }

            // 데이터를 캐시에 저장
            cache.translationCache.set(cacheKey, translatedData);

            // JSON 형식의 데이터를 그대로 반환
            res.json(translatedData);
        } catch (error) {
            console.error('Failed to load data from record.json:', error);
            res.status(500).send('데이터를 불러오는데 실패했습니다.');
        }
    });
